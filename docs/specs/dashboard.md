...
The dashboard is the V2 read side of Critiq. GitHub PRs remain the primary
interface, but the dashboard surfaces what Critiq knows and has done:
...
It is **read-only** for now. All writes happen through the webhook + worker.
...
| `GET /dashboard/runs/{id}` | Run detail | Run metadata, summary, error, all findings |
...
- Per-finding feedback (accept/reject/resolve) from this UI, feeding the
  learning loop
